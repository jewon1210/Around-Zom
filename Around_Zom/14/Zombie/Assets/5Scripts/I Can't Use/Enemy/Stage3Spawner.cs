...
public class Stage3Spawner : MonoBehaviour
{
    ...
    public Transform Daughter_spawn;//딸의 스폰위치

    bool donwave; //wave 실행 ㄴㄴ
    ...
    public GameObject daughter; // 딸 오브젝트
    ...
    private void Update()
    {
        ...
        if (enemies.Count <= 0 && wave != 7)
        {
            SpawnWave();
        }

        if (wave == 7 && enemies.Count <= 0 && !BossSpawned && donwave ==false)
        {
            StageBossSpawn();
            BossSpawned = true;
        }
        ...
        if (IsBossDead && !donwave)
        {
            daughter.SetActive(true); //딸 소환이 중요!
            //Daughter.Translate(Mother.position.x, Mother.position.y, Mother.position.z);
            //Daughter.position = new Vector3(Daughter_spawn.position.x, Daughter_spawn.position.y, Daughter_spawn.position.z);
            donwave = true;
        }
    ...
    void StageBossSpawn()
    {
        ...
        Color skinColor = new Color(0.7749004f, 0.9622641f, 0.0408508f);
